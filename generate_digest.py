...
# Эта подпись оказалась меткой источника (например, при цитировании чужого
# контента), а не тэглайном одного конкретного канала — встречается в
# разных каналах, поэтому убираем её у ВСЕХ, а не только там, где её
# явно прописали в настройках канала.
GLOBAL_PREFIXES_TO_STRIP = ["Банки, деньги, два офшора"]

# Известные источники, на которые часто ссылаются в конце новости через тире:
# "...текст — РБК." Список неполный и его можно пополнять.
KNOWN_SOURCE_NAMES = [
...
# "...текст — РБК." / "...текст – Baza" — тире и точное имя известного источника в конце.
SOURCE_DASH_SUFFIX_RE = re.compile(
    rf"\s*[—–-]\s*(?:{_sources_escaped})\.?\s*$", re.IGNORECASE
)

# "...текст, пишет Baza." / "...сообщают Известия." — глагол атрибуции + название
# источника (любое, не только из списка выше) в конце предложения.
...
def strip_source_attribution(text: str) -> str:
    """Убирает ссылку на источник в конце новости ('— РБК.', 'пишет Baza.').
    Работает и на исходном тексте поста, и на готовом резюме."""
    if not text:
        return text
    for _ in range(2):  # на случай двойной атрибуции подряд
        before = text
...
def _looks_russian(texts: list) -> bool:
    """Проверка, что резюме реально на русском (а не внезапно на английском —
    так иногда ведут себя бесплатные модели). Не идеальна, но отсекает случаи,
    когда LLM целиком проигнорировал инструкцию про язык."""
...
def algorithmic_summary(text: str) -> str:
    ...
    result = first
    # Второе предложение добавляем только если первое совсем короткое —
    # иначе резюме без LLM получается длиннее, чем нужно.
    if len(sentences) > 1 and len(first) < 60:
        result = f"{first} {sentences[1]}".strip()
    if len(result) > 280:
        result = result[:277].rstrip() + "..."
    result = strip_source_attribution(result)
...
def text_fingerprint(text: str) -> str:
    """Нормализованный 'отпечаток' текста поста для дедупликации по содержанию —
    два источника (rss-bridge и i-c-a.su) иногда отдают ОДИН И ТОТ ЖЕ пост
    под разными msgid (например, из-за медиа-групп/альбомов), из-за чего
    дедупликация по ссылке этого не ловит."""
...
def collect_posts() -> dict:
    ...
    # Отпечатки текстов, уже лежащих в архиве, по каналам — чтобы не
    # заносить один и тот же пост дважды под разными msgid.
    seen_fingerprints = {}
    for v in archive.values():
        seen_fingerprints.setdefault(v.get("channel"), set()).add(text_fingerprint(v.get("text", "")))
    ...
    for channel_name, channel_cfg in CHANNELS.items():
        for url in channel_cfg["sources"]:
            ...
            for entry in parsed.entries:
                ...
                text = strip_known_prefixes(text, GLOBAL_PREFIXES_TO_STRIP + channel_cfg.get("text_prefix_strip", []))
                text = re.sub(r"\s+", " ", text).strip()

                if len(text) < 10:
                    # Реального текста не осталось (пост только с медиа/заглушкой
                    # вложения) — просто не архивируем такой пост, а не пишем
                    # плейсхолдер, который потом попадёт в дайджест как "новость".
                    skipped_no_text += 1
                    continue

                fp = text_fingerprint(text)
                channel_fps = seen_fingerprints.setdefault(channel_name, set())
                if fp in channel_fps:
                    # Тот же текст уже есть в архиве под другим msgid (обычно из-за
                    # второго источника или медиа-группы) — не дублируем.
                    skipped_duplicate += 1
                    continue
    ...
    log(
        ...
        f"Пропущено (без текста): {skipped_no_text}. Пропущено (дубли по тексту): {skipped_duplicate}. "
...
def build_prompt(posts: list, combine: bool, extra_instructions: str) -> str:
    ...
    base = (
        ...
        "ВАЖНО: независимо от того, на каком языке написан исходный пост, "
        "резюме ВСЕГДА пиши ТОЛЬКО на русском языке — если пост на английском "
        "или любом другом языке, переведи суть на русский, не оставляй "
        "английский текст в ответе. "
        "Не указывай в резюме источник новости (например 'пишет РБК', "
        "'сообщает Известия', 'по данным Baza', '— The Times of India') — "
        "оставляй только суть самого события, без ссылки на то, кто об этом написал. "
    )
    ...
    if combine:
        base += (
            ...
            "В большинстве случаев каждое резюме — одно ёмкое предложение. "
            "Используй два предложения ТОЛЬКО если без второго теряются "
            "критически важные конкретные цифры, суммы, даты или причины — "
            "такие случаи должны быть редкими, а не правилом. "
            "Не добавляй вводных фраз, не пересказывай весь пост целиком. "
        ...
        base += (
            ...
            "передающее суть новости. В большинстве случаев — одно ёмкое "
            "предложение. Используй два предложения ТОЛЬКО если без второго "
            "теряются критически важные конкретные цифры, суммы, даты или "
            "причины — такие случаи должны быть редкими, а не правилом. "
            "Не добавляй вводных фраз, не пересказывай весь пост целиком. "
...
def _parse_flat(content: str, n: int):
    ...
    if isinstance(data, list) and len(data) == n and all(isinstance(x, str) for x in data):
        cleaned = [strip_source_attribution(s.strip()) for s in data]
        if not _looks_russian(cleaned):
            return None
        return cleaned
    return None
    ...
    for item in data:
        ...
        for i in idxs:
            if i < 1 or i > n or i in covered:
                return None
            covered.add(i)
        groups.append({"summary": strip_source_attribution(summary.strip()), "posts": idxs})
    if covered != set(range(1, n + 1)):
        return None
    if not _looks_russian([g["summary"] for g in groups]):
        return None
    return groups
...
def sanitize_archived_text(text: str, channel_cfg: dict):
    """Повторно применяет ТЕКУЩИЕ правила чистки к тексту, который уже лежит
    в архиве. Это нужно потому, что часть постов могла быть заархивирована
    более старой версией скрипта — до того как появились те или иные
    правила (удаление рекламы, служебных фраз Telegram, пересланных
    сообщений и т.п.). Без этого шага такие старые записи навсегда
    оставались бы "грязными", даже после замены файла на новую версию.

    Возвращает очищенный текст, либо None — если после чистки реального
    текста не осталось (пост нужно полностью исключить из дайджеста, а не
    ...
    # Заглушка старых версий скрипта — такой пост исключаем целиком.
...
def try_build_digest(archive: dict) -> None:
    ...
    for channel_name, channel_cfg in CHANNELS.items():
        ...
        if not posts_today_raw:
            log(f"[{channel_name}] Нет постов за {target_date} — {feed_path} не трогаю.")
            continue

        posts_today = []
        skipped_on_build = 0
        for p in posts_today_raw:
            clean = sanitize_archived_text(p.get("text", ""), channel_cfg)
            if clean is None:
                skipped_on_build += 1
                continue
            p2 = dict(p)
            p2["text"] = clean
            posts_today.append(p2)

        if skipped_on_build:
            log(f"[{channel_name}] Исключено при повторной проверке (мусор/реклама/пересланное): {skipped_on_build}.")

        if not posts_today:
            log(f"[{channel_name}] После чистки не осталось постов за {target_date} — {feed_path} не трогаю.")
            continue

        # Дедупликация по содержанию: если два источника уже успели занести в
        # архив один и тот же пост под разными msgid ДО того, как появилась
        # проверка при сборе, здесь мы это тоже отловим и уберём повтор.
        seen_fp = set()
        deduped = []
        duplicates_removed = 0
        for p in posts_today:
            fp = text_fingerprint(p["text"])
            if fp in seen_fp:
        ...
        if duplicates_removed:
            log(f"[{channel_name}] Убрано дублей по содержанию: {duplicates_removed}.")
        posts_today = deduped
